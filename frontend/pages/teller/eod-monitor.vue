...
const fetchDateRangeData = async () => {
  ...
  try {
    // Fetch range data
    const rangeResponse = await fetch(`http://localhost:9000/api/eod/range?startDate=${startDate.value}&endDate=${endDate.value}`, {
      headers: {
    ...
    const perfResponse = await fetch(`http://localhost:9000/api/eod/performance?startDate=${startDate.value}&endDate=${endDate.value}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    });

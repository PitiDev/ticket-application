...
const fetchTemplates = async () => {
  try {
    const response = await fetch('http://localhost:9000/api/eod/templates', {
      headers: {
...
const fetchBranches = async () => {
  try {
    const response = await fetch('http://localhost:9000/api/branches', {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    });
...
const fetchTemplateItems = async () => {
  ...
  try {
    const response = await fetch(`http://localhost:9000/api/eod/templates/${formData.value.template_id}/items`, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    });
...
const createSession = async () => {
  ...
  try {
    const response = await fetch('http://localhost:9000/api/eod/sessions', {
      method: 'POST',
      headers: {

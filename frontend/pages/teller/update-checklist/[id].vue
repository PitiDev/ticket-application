...
const fetchChecklistDetails = async () => {
  ...
  try {
    const response = await fetch(`http://localhost:9000/api/eod/checklist/${checklistId.value}`, {
      headers: {
  ...
  try {
    ...
    for (const [itemId, file] of Object.entries(itemAttachments.value)) {
      if (file) {
        ...
        const uploadResponse = await fetch('http://localhost:9000/api/eod/attachments', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`
          },

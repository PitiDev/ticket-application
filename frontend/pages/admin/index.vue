...
import {
  ...
  DialogTitle,
  TransitionRoot
} from '@headlessui/vue' // เพิ่มบรรทัดนี้
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div v-if="loading" class="flex justify-center py-12">
        <div class="flex flex-col items-center gap-3">
          <ArrowPathIcon class="h-10 w-10 animate-spin text-indigo-600" />
                ...
                <tr v-for="user in filteredUsers" :key="user.id" class="hover:bg-gray-50/50 transition-colors">
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                      <div
                        class="h-10 w-10 rounded-full bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center text-white font-medium">
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                      <button @click="editUser(user)" class="text-gray-400 hover:text-indigo-600 transition-colors"
                        title="Edit user">
      ...
      <div v-else-if="activeTab === 'departments'" class="space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
          <div v-if="filteredDepartments.length === 0" class="p-8 text-center text-gray-500">
            <BuildingOfficeIcon class="h-12 w-12 mx-auto text-gray-400 mb-3" />
          ...
          <div v-else class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="divide-y divide-gray-200">
                <tr v-for="dept in filteredDepartments" :key="dept.id" class="hover:bg-gray-50/50 transition-colors">
                  <td class="px-6 py-4">
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-2">
                      ...
                      <span class="text-sm text-gray-900">{{ dept.user_count || 0 }}</span>
                    </div>
                  </td>
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                      <button @click="editDepartment(dept)"
                        class="text-gray-400 hover:text-indigo-600 transition-colors" title="Edit department">
                        <PencilSquareIcon class="h-5 w-5" />
                      </button>
          ...
          <div class="space-y-6">
            <div v-for="role in ['super_admin', 'admin', 'manager', 'agent', 'user']" :key="role"
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                ]" :key="idx" class="flex items-center justify-between p-3 bg-gray-50 rounded">
                  <span class="text-sm text-gray-700">{{ perm.name }}</span>
                  <span :class="perm.enabled ? 'text-green-600' : 'text-gray-400'">
            ...
            <p class="text-sm text-gray-500 mt-4">
              Note: This is a simplified view of role permissions. Only Super Admin can modify system-wide permissions.
            </p>
          </div>
        </div>
      </div>
          ...
          <DialogPanel class="w-full max-w-md bg-white rounded-xl shadow-lg p-6">
            ...
            <form @submit.prevent="handleAddUser" class="space-y-4">
              ...
              <div>
                ...
                <select v-model="userForm.role" class="mt-1 block w-full rounded-lg border border-gray-300 px-3 py-2"
                  required>
                  <option value="user">User</option>
                  <option value="agent">Agent</option>
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
          <DialogPanel class="w-full max-w-md bg-white rounded-xl shadow-lg p-6">
            ...
            <p class="text-gray-600 mb-4">
              Are you sure you want to delete user <span class="font-semibold">{{ selectedUser?.username }}</span>? This
      ...
      <Dialog as="div" @close="showDeleteDeptModal = false" class="relative z-50">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
          <DialogPanel class="w-full max-w-md bg-white rounded-xl shadow-lg p-6">
            ...
            <p class="text-gray-600 mb-4">
              Are you sure you want to delete department <span class="font-semibold">{{ selectedDepartment?.name

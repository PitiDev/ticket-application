<template>
    <div class="gold-report">
        ...
        <div class="bg-white rounded-xl shadow-md mb-6">
            <div class="border-b border-gray-200">
                <nav class="flex -mb-px">
                    <button @click="activeTab = 'gold-report'" :class="{
                        'border-amber-500 text-amber-600': activeTab === 'gold-report',
                        'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'gold-report'
                    }" class="w-1/2 py-4 px-1 text-center border-b-2 font-medium text-sm transition-colors">
                        📊 Gold Investment Report
                    </button>
                    <button @click="activeTab = 'lbb-trading'" :class="{
                        'border-amber-500 text-amber-600': activeTab === 'lbb-trading',
                        'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300': activeTab !== 'lbb-trading'
                    }" class="w-1/2 py-4 px-1 text-center border-b-2 font-medium text-sm transition-colors">
                        📈 LBB Trading View
                    </button>
                </nav>
        ...
        <!-- Gold Report Tab -->
        <div v-show="activeTab === 'gold-report'">
            <!-- Customer ID Search -->
            ...
            <div v-if="loading" class="flex justify-center items-center h-96">
                <div class="animate-pulse text-center">
                    ...
                    <div class="inline-block w-12 h-12 border-4 border-t-amber-400 border-amber-200 rounded-full animate-spin"></div>
                </div>
            </div>
            ...
            <!-- Report content - MOVED REF TO WRAPPER -->
            <div v-else-if="dataLoaded && hasData">
                <div ref="reportContent" class="space-y-8">
                    <!-- Header -->
                        ...
                        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                            <div>
                                ...
                                <div v-if="customerAccounts.accounts && customerAccounts.accounts.length > 0"
                                    ...
                                    <div class="space-y-1">
                                        <div v-for="(account, index) in customerAccounts.accounts" :key="index" class="flex text-sm">
                            ...
                            <div class="bg-white/20 backdrop-blur p-4 rounded-lg text-center">
                                <div class="text-sm uppercase tracking-wide mb-1">Current Gold Price (BUY)</div>
                        ...
                        <div class="bg-white rounded-xl p-6 shadow-md">
                            <div class="flex justify-between">
                                <div>
                                    ...
                                    <div :class="{ 'text-2xl font-bold': true, 'text-green-600': goldData.summary?.is_overall_profitable, 'text-red-600': !goldData.summary?.is_overall_profitable }">
                                        {{ goldData.summary?.total_profit_loss }} ({{ goldData.summary?.total_profit_loss_percentage }})
                                    </div>
                                </div>
                                <div :class="{ 'p-3 rounded-full': true, 'bg-green-100': goldData.summary?.is_overall_profitable, 'bg-red-100': !goldData.summary?.is_overall_profitable }">
                                    <svg v-if="goldData.summary?.is_overall_profitable" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <section class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        <div class="lg:col-span-2 bg-white rounded-xl p-6 shadow-md">
                            ...
                            <div class="h-64">
                                <ClientOnly>
                                    <GoldPerformanceChart 
                                        v-if="goldData.transactions && goldData.transactions.length > 0"
                        ...
                        <div class="bg-white rounded-xl p-6 shadow-md">
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="h-36 mt-4">
                                    <ClientOnly>
                                        <GoldPortfolioDonut 
                                            v-if="goldData.summary?.total_gold_weight"
                                            :totalGold="parseFloat(goldData.summary.total_gold_weight)"
                                            :profitableWeight="parseFloat(goldData.recommendations?.[1]?.profitable_gold_weight || 0)" 
                                        />
                                        <div v-else class="flex items-center justify-center h-full text-gray-400">
                                            Loading...
                                        </div>
                    ...
                    <section v-if="hasP2PData" class="bg-white rounded-xl p-6 shadow-md">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            <div class="bg-indigo-50 p-4 rounded-lg">
                                ...
                                <div class="text-xl font-bold text-indigo-800">{{ p2pGoldData.summary?.total_transactions }}</div>
                            </div>
                            <div class="bg-indigo-50 p-4 rounded-lg">
                                <div class="text-sm text-gray-600 mb-1">Total Gold Transferred</div>
                                <div class="text-xl font-bold text-indigo-800">{{ p2pGoldData.summary?.total_gold_weight }}</div>
                            </div>
                            <div class="bg-indigo-50 p-4 rounded-lg">
                                <div class="text-sm text-gray-600 mb-1">Received / Sent</div>
                                <div class="text-xl font-bold text-indigo-800">{{ getP2PTransferCounts.received }} / {{ getP2PTransferCounts.sent }}</div>
                            </div>
                        </div>
                        <div class="overflow-x-auto mt-6">
                            ...
                            <table class="min-w-full">
                                <thead class="bg-gray-50">
                                    <tr>
                                        <th class="px-1 py-1 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">#</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Transaction ID</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Date</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Gold Weight</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="(transaction, index) in p2pGoldData.transactions" :key="index" class="hover:bg-gray-50">
                                        <td class="px-1 py-1 whitespace-nowrap text-sm font-medium text-gray-900">{{ index + 1 }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ transaction.transaction_id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ transaction.transfer_date }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ transaction.gold_weight }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span :class="{ 'px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full': true, 'bg-green-100 text-green-800': isIncomingTransfer(transaction), 'bg-blue-100 text-blue-800': !isIncomingTransfer(transaction) }">
                                                {{ isIncomingTransfer(transaction) ? 'Received' : 'Sent' }}
                                                ({{ isIncomingTransfer(transaction) ? 'Incoming' : 'Outgoing' }})
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span :class="{ 'text-gray-500': true, 'font-semibold': isMyAccount(transaction.debit_account) }">
                                                {{ transaction.debit_account }}
                                                <span v-if="isMyAccount(transaction.debit_account)" class="text-xs text-blue-600 ml-1">(My Account)</span>
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span :class="{ 'text-gray-500': true, 'font-semibold': isMyAccount(transaction.credit_account) }">
                                                {{ transaction.credit_account }}
                                                <span v-if="isMyAccount(transaction.credit_account)" class="text-xs text-blue-600 ml-1">(My Account)</span>
                                            </span>
                                        </td>
                                    </tr>
                                </tbody>
                    ...
                    <section v-if="hasSellData" class="bg-white rounded-xl p-6 shadow-md">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                            <div class="bg-amber-50 p-4 rounded-lg">
                                ...
                                <div class="text-xl font-bold text-amber-800">{{ sellGoldData.summary?.total_transactions }}</div>
                            </div>
                            <div class="bg-amber-50 p-4 rounded-lg">
                                <div class="text-sm text-gray-600 mb-1">Total Gold Sold</div>
                                <div class="text-xl font-bold text-amber-800">{{ sellGoldData.summary?.total_gold_weight }}</div>
                            </div>
                            <div class="bg-amber-50 p-4 rounded-lg">
                                <div class="text-sm text-gray-600 mb-1">Total Amount</div>
                                <div class="text-xl font-bold text-amber-800">{{ sellGoldData.summary?.total_sold_amount }}</div>
                            </div>
                        </div>
                        <div class="overflow-x-auto mt-6">
                            ...
                            <table class="min-w-full">
                                <thead class="bg-gray-50">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Gold Weight</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Price/Gram</th>
                                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Amount</th>
                                    </tr>
                                </thead>
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="(transaction, index) in sellGoldData.transactions" :key="index" class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ transaction.transaction_id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ transaction.sale_date }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <span :class="{ 'text-gray-500': true, 'font-semibold': isMyAccount(transaction.account_no) }">
                                                {{ transaction.account_no }}
                                                <span v-if="isMyAccount(transaction.account_no)" class="text-xs text-blue-600 ml-1">(My Account)</span>
                                            </span>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ transaction.gold_weight }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ transaction.price_per_gram }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-red-600">{{ transaction.total_amount }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <section v-if="hasSellData || hasP2PData" class="bg-white rounded-xl p-6 shadow-md">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                            ...
                            <div :class="{ 'p-5 rounded-lg text-white': true, 'bg-gradient-to-r from-green-500 to-emerald-600': netPositionProfitLoss.isProfit, 'bg-gradient-to-r from-red-500 to-rose-600': !netPositionProfitLoss.isProfit }">
                    ...
                    <section class="bg-white rounded-xl p-6 shadow-md">
                        ...
                        <div v-if="aiAnalysisLoading" class="flex items-center justify-center h-48">
                            <div class="animate-pulse text-center">
                                ...
                                <div class="inline-block w-8 h-8 border-2 border-t-blue-400 border-blue-200 rounded-full animate-spin"></div>
                            </div>
                        </div>
                        <div v-else-if="!aiAnalysis && !aiAnalysisLoading" class="flex flex-col items-center justify-center h-48">
                                ...
                                <table class="min-w-full">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-left text-xs font-medium text-gray-500 uppercase tracking-wider py-2">Weight</th>
                                            <th class="text-right text-xs font-medium text-gray-500 uppercase tracking-wider py-2">Profit</th>
                                            <th class="text-right text-xs font-medium text-gray-500 uppercase tracking-wider py-2">%</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr v-for="(transaction, index) in goldData.recommendations[0].transactions" :key="index" class="border-t border-amber-100">
                                            <td class="py-2 text-sm">{{ transaction.purchase_date }}</td>
                                            <td class="py-2 text-sm">{{ transaction.gold_weight }}</td>
                                            <td class="py-2 text-sm text-right font-medium text-green-600">{{ transaction.profit }}</td>
                                            <td class="py-2 text-sm text-right font-medium text-green-600">{{ transaction.profit_percentage }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                        ...
                        <div v-if="goldData.recommendations?.[1]" class="bg-white rounded-xl p-6 shadow-md">
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div class="bg-amber-50 rounded-lg p-4 text-center">
                                    <div class="text-3xl font-bold text-amber-600">{{ goldData.recommendations[1].overall_profit_percentage }}</div>
                                    <div class="text-sm text-gray-600">Overall Profit</div>
                                </div>
                                <div class="bg-amber-50 rounded-lg p-4 text-center">
                                    <div class="text-3xl font-bold text-amber-600">{{ goldData.recommendations[1].total_gold_weight }}</div>
                    ...
                    <section class="bg-white rounded-xl p-6 shadow-md">
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="(transaction, index) in displayedTransactions" :key="index" class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ index + 1 }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ transaction.transaction_id }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ transaction.purchase_date }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ transaction.gold_weight }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ transaction.purchase_price_per_gram }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ transaction.current_price_per_gram }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <span :class="{ 'text-sm font-medium': true, 'text-green-600': transaction.is_profitable, 'text-red-600': !transaction.is_profitable }">
        ...
        <!-- LBB Trading View Tab -->
        <div v-show="activeTab === 'lbb-trading'">
            <ClientOnly>
                <LbbTradingView />
                <template #fallback>
                    <div class="flex justify-center items-center h-96">
                        <div class="text-gray-500">Loading Trading View...</div>
                    </div>
                </template>
            </ClientOnly>
        </div>
...
import LbbTradingView from './lbb-trading-view.vue'
...
const activeTab = ref('gold-report')

// Computed
const canExport = computed(() => dataLoaded.value && hasData.value && activeTab.value === 'gold-report')
...
const hasSellData = computed(() => sellGoldData.value?.summary?.total_transactions > 0)
const hasP2PData = computed(() => p2pGoldData.value?.summary?.total_transactions > 0)
...
const getP2PTransferCounts = computed(() => {
    ...
    p2pGoldData.value.transactions.forEach(transaction => {
        if (isIncomingTransfer(transaction)) received++
        else sent++
    })
...
const calculateP2PNetWeight = computed(() => {
    ...
    let incomingWeight = 0, outgoingWeight = 0
    p2pGoldData.value.transactions.forEach(transaction => {
        const weight = parseFloat(transaction.gold_weight?.replace(/[^\d.-]/g, '') || 0)
        if (isIncomingTransfer(transaction)) incomingWeight += weight
        else outgoingWeight += weight
    })
    return `${(incomingWeight - outgoingWeight).toFixed(2)} grams`
})
...
const calculateTotalNetWeight = computed(() => {
    ...
    let p2pWeight = 0
    if (hasP2PData.value) {
        p2pWeight = parseFloat(calculateP2PNetWeight.value.replace(/[^\d.-]/g, '') || 0)
    }
...
const formatNetGoldValue = computed(() => {
    if (!goldData.value?.summary?.current_gold_price?.buy_rate) return 'N/A'
    const netWeight = parseFloat(calculateTotalNetWeight.value.replace(/[^\d.-]/g, '') || 0)
    const priceStr = goldData.value.summary.current_gold_price.buy_rate
    const match = priceStr.match(/[\d,]+\.\d+|\d+/)
    if (!match) return 'N/A'
    const price = parseFloat(match[0].replace(/,/g, ''))
    return formatCurrency(netWeight * price)
})
...
const netPositionProfitLoss = computed(() => {
    ...
    let totalInvestment = 0, totalWeight = 0
...
async function fetchData(customerId) {
    ...
    try {
        ...
        if (sellingReportResponse.ok) sellingData = await sellingReportResponse.json()

        let p2pData = { data: { summary: { total_transactions: 0 }, transactions: [] } }
        if (p2pReportResponse.ok) p2pData = await p2pReportResponse.json()

        let accountsData = { data: { customer_info: { total_accounts: 0 }, accounts: [] } }
        if (customerAccountsResponse.ok) accountsData = await customerAccountsResponse.json()
...
const exportToPDF = async () => {
    if (!canExport.value || !reportContent.value) {
        console.warn('Cannot export: report not ready')
        return
    }

    if (activeTab.value !== 'gold-report') {
        activeTab.value = 'gold-report'
        await nextTick()
    }
...
const exportToExcel = () => {
    if (!canExport.value) {
        console.warn('Cannot export: data not ready')
        return
    }
    ...
    if (hasSellData.value) {
        summaryData.push(['', ''], ['Selling Summary', null], 
            ['Total Sell Transactions', sellGoldData.value.summary?.total_transactions],
            ['Total Gold Sold', sellGoldData.value.summary?.total_gold_weight],
            ['Total Sold Amount', sellGoldData.value.summary?.total_sold_amount],
            ['Average Price Per Gram', sellGoldData.value.summary?.average_price_per_gram])
    }

    if (hasP2PData.value) {
        summaryData.push(['', ''], ['P2P Transfers Summary', null],
            ['Total P2P Transactions', p2pGoldData.value.summary?.total_transactions],
            ['Total Gold Transferred', p2pGoldData.value.summary?.total_gold_weight],
            ['Transfers Received', getP2PTransferCounts.value.received],
            ['Transfers Sent', getP2PTransferCounts.value.sent],
            ['P2P Net Weight', calculateP2PNetWeight.value])
    }

    if (hasSellData.value || hasP2PData.value) {
        summaryData.push(['', ''], ['Net Position', null],
            ['Net Gold Weight', calculateTotalNetWeight.value])
        if (hasSellData.value) {
    ...
    // Transactions Sheet
    const transactionHeaders = ['Transaction ID', 'Date', 'Gold Weight', 'Purchase Price', 'Current Price', 'Total Purchase', 'Current Value', 'Profit/Loss', 'P/L %', 'Profitable']
    const transactionData = [transactionHeaders]

    if (goldData.value.transactions) {
        goldData.value.transactions.forEach(t => {
            transactionData.push([
                t.transaction_id, t.purchase_date, t.gold_weight, t.purchase_price_per_gram,
                t.current_price_per_gram, t.total_purchase_amount, t.current_value,
                t.profit_loss, t.profit_loss_percentage, t.is_profitable ? 'Yes' : 'No'
            ])
        })
    }
    ...
    if (hasSellData.value && sellGoldData.value.transactions.length > 0) {
        const sellHeaders = ['Transaction ID', 'Date', 'Account', 'Is My Account', 'Gold Weight', 'Price Per Gram', 'Total Amount', 'Currency']
        const sellData = [sellHeaders]
        sellGoldData.value.transactions.forEach(t => {
            sellData.push([
                t.transaction_id, t.sale_date, t.account_no, isMyAccount(t.account_no) ? 'Yes' : 'No',
                t.gold_weight, t.price_per_gram, t.total_amount, t.currency
            ])
        })
    ...
    // P2P Transactions Sheet
    if (hasP2PData.value && p2pGoldData.value.transactions.length > 0) {
        const p2pHeaders = ['Transaction ID', 'Date', 'Gold Weight', 'Direction', 'Type', 'From Account', 'Is My Account (From)', 'To Account', 'Is My Account (To)', 'Reference']
        const p2pData = [p2pHeaders]
        p2pGoldData.value.transactions.forEach(t => {
            p2pData.push([
                t.transaction_id, t.transfer_date, t.gold_weight,
                isIncomingTransfer(t) ? 'Incoming' : 'Outgoing',
                isIncomingTransfer(t) ? 'Received' : 'Sent',
                t.debit_account, isMyAccount(t.debit_account) ? 'Yes' : 'No',
                t.credit_account, isMyAccount(t.credit_account) ? 'Yes' : 'No',
                t.core_banking_ref
            ])
        })
        const p2pSheet = XLSX.utils.aoa_to_sheet(p2pData)
        XLSX.utils.book_append_sheet(wb, p2pSheet, 'P2P Transfers')
    }

    // Customer Accounts Sheet
    if (customerAccounts.value.accounts && customerAccounts.value.accounts.length > 0) {
        const accountHeaders = ['Account Number', 'Account Name', 'Account Type', 'Currency']
        const accountData = [accountHeaders]
        customerAccounts.value.accounts.forEach(account => {
            accountData.push([account.account_no, account.account_name, account.account_type, account.account_currency])
        })
    ...
    if (goldData.value.recommendations && goldData.value.recommendations.length > 0) {
        ...
        if (goldData.value.recommendations[0].transactions) {
            goldData.value.recommendations[0].transactions.forEach(t => {
                recData.push([t.transaction_id, t.purchase_date, t.gold_weight, t.profit, t.profit_percentage])
            })
        }
...
onMounted(() => {
    ...
    if (initialCustomerId) fetchData(initialCustomerId)
})

watch(searchCustomerId, (newId) => {
    if (newId) router.replace({ query: { id: newId } })
})
</script>

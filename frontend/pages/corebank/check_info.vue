  ...
  <div class="min-h-screen bg-gray-50 py-8 px-4">
    <div class="max-w-6xl mx-auto">
      ...
      <div class="bg-white rounded-lg shadow-md mb-6 overflow-hidden">
        <div class="border-b border-gray-200">
          <nav class="flex space-x-8 px-6 overflow-x-auto" aria-label="Tabs">
            <button v-for="tab in searchTabs" :key="tab.id" @click="activeTab = tab.id" :class="[
        ...
        <div class="p-6">
          ...
          <div v-if="activeTab === 'phone'" class="space-y-4">
            <div>
              ...
              <input id="phoneNumber" v-model="searchData.phoneNumber" type="tel"
          ...
          <div v-if="activeTab === 'idphone'" class="space-y-4">
            <div class="grid md:grid-cols-2 gap-4">
              <div>
                ...
                <input id="globalId" v-model="searchData.globalId" type="text"
              ...
              <div>
                ...
                <input id="phoneNumberCombo" v-model="searchData.phoneNumberCombo" type="tel"
                  placeholder="Enter phone number (e.g., 2097778968)"
                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-colors"
                  @keyup.enter="searchClientInfo" />
              </div>
          ...
          <div v-if="activeTab === 'name'" class="space-y-4">
            <div>
              ...
              <input id="clientName" v-model="searchData.clientName" type="text"
          ...
          <div v-if="activeTab === 'accounts'" class="space-y-4">
            <div>
              ...
              <input id="customerId" v-model="searchData.customerId" type="text"
          ...
          <div v-if="activeTab === 'comparison'" class="space-y-4">
            <div>
              ...
              <input id="comparisonPhone" v-model="searchData.comparisonPhone" type="tel"
          ...
          <div class="flex justify-center mt-6">
            <button @click="searchClientInfo" :disabled="pending || !isValidSearch"
      ...
      <div v-if="clientData && !pending" class="space-y-6">
        ...
        <div v-if="activeTab === 'accounts' && clientData?.format === 'raw'"
          class="bg-white rounded-lg shadow-md overflow-hidden border-l-4 border-indigo-500">
          ...
          <div class="p-6">
            <div class="grid md:grid-cols-4 gap-4 text-center">
              ...
              <div class="bg-amber-50 p-4 rounded-lg">
                <p class="text-2xl font-bold text-amber-600">{{ Math.round(((getAccountsData?.summary.total_accounts ||
                  0) / clientData.total_records) * 100) }}%</p>
            ...
            <div class="grid md:grid-cols-2 gap-6">
              <div class="space-y-4">
                <div>
                  ...
                  <p class="text-lg font-semibold text-gray-900">{{ getClientInfo?.client_no || (activeTab ===
                    'comparison' ? clientData.customer_info.core_banking.client_no : '') }}</p>
                </div>
                <div>
                  <label class="text-sm font-medium text-gray-500">Client Name</label>
                  <p class="text-lg font-semibold text-gray-900">{{ getClientInfo?.client_name || (activeTab ===
                    'comparison' ? clientData.customer_info.core_banking.client_name : '') }}</p>
                </div>
                <div>
                  <label class="text-sm font-medium text-gray-500">Client ID</label>
                  <p class="text-lg font-semibold text-gray-900">{{ getClientInfo?.client_id || (activeTab ===
                    'comparison' ? clientData.customer_info.core_banking.client_id : '') }}</p>
                </div>
              </div>
              <div class="space-y-4">
                <div>
                  <label class="text-sm font-medium text-gray-500">Global ID Type</label>
                  <p class="text-lg font-semibold text-gray-900">{{ getClientInfo?.global_id_type || (activeTab ===
                    'comparison' ? clientData.customer_info.core_banking.global_id_type : '') }}</p>
                </div>
                <div>
                  <label class="text-sm font-medium text-gray-500">Global ID</label>
                  <p class="text-lg font-semibold text-gray-900">{{ getClientInfo?.global_id || (activeTab ===
                    'comparison' ? clientData.customer_info.core_banking.global_id : '') }}</p>
                </div>
                <div>
                  <label class="text-sm font-medium text-gray-500">Created By</label>
                  <p class="text-lg font-semibold text-gray-900">{{ getClientInfo?.created_by || (activeTab ===
                    'comparison' ? clientData.customer_info.core_banking.created_by : '') }}</p>
                </div>
              </div>
            </div>
                ...
                <div class="space-y-4">
                  <div>
                    ...
                    <p class="text-lg font-semibold text-gray-900">{{
                      clientData.customer_info.mobile_banking.customer_id }}</p>
                  </div>
                </div>
                <div class="space-y-4">
                  <div>
                    ...
                    <p class="text-lg font-semibold text-gray-900">{{
                      clientData.customer_info.mobile_banking.total_accounts }}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div v-if="activeTab === 'comparison' && clientData.summary"
          ...
          <div class="p-6">
            <div class="grid md:grid-cols-4 gap-4 text-center">
              <div class="bg-blue-50 p-4 rounded-lg">
                <p class="text-2xl font-bold text-blue-600">{{ clientData.summary.total_accounts_in_mobile_banking }}
                </p>
            ...
            <div class="mt-6 pt-4 border-t border-gray-200">
              <div class="flex gap-3 mb-3">
                <span class="px-3 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">
                  {{ Math.round((clientData.summary.matching_accounts /
                    clientData.summary.total_accounts_in_core_banking) * 100) }}% Sync Rate
                </span>
                <span v-if="clientData.contact_details?.phone_number_match"
        ...
        <div v-if="activeTab === 'accounts' && clientData.summary"
          class="bg-white rounded-lg shadow-md overflow-hidden">
        ...
        <div v-if="activeTab === 'comparison' && clientData.AccountMobile && clientData.AccountCoreBank"
          class="bg-white rounded-lg shadow-md overflow-hidden">
              ...
              <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col"
                      class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Currency
                    </th>
                    <th scope="col"
                      class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                    <th scope="col"
                      class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">System</th>
                    <th scope="col"
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                  ...
                  <template v-for="(account, index) in clientData.AccountMobile.accounts" :key="`mobile-${index}`">
                    <tr class="bg-blue-50 hover:bg-blue-100 transition-colors">
                      <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ account.account_no }}
                      </td>
                  ...
                  <template v-for="(account, index) in clientData.AccountCoreBank.accounts" :key="`core-${index}`">
                    <tr :class="[
                      ...
                      <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                        {{ account.account_type === 'C' ? 'CURRENT' : (account.account_type === 'T' ? 'TD12' :
                      ...
                      <td class="px-6 py-4 whitespace-nowrap">
                        <span :class="[
                          ...
                          {{ account.status === 'A' ? 'Active' : (account.status === 'N' ? 'New/Inactive' :
                            account.status) }}
                        </span>
                      </td>
                      <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-2">
                          ...
                          <span v-if="!account.exists_in_mobile"
                      ...
                      <td class="px-6 py-4 whitespace-nowrap">
                        ...
                        <button v-if="!account.exists_in_mobile" @click="addAccountToMobile(account)"
                          ...
                          <svg v-if="addingAccount === account.account_no" class="animate-spin h-3 w-3" fill="none"
                            viewBox="0 0 24 24">
        ...
        <div v-if="activeTab === 'accounts' && clientData.accounts"
          class="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div class="p-6">
            <div v-if="clientData.accounts?.length > 0" class="space-y-3">
              <div v-for="account in clientData.accounts" :key="account.acct_no"
                ...
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-gray-600">{{ account.ccy }} • {{ account.deposit_type === 'C' ? 'Current' :
                      'Term' }}
                      | {{ getAccountTypeName(account.acct_type) }}</p>
                  </div>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div class="p-6">
            <div v-if="getContactDetails?.length > 0" class="space-y-3">
              <div v-for="contact in getContactDetails" :key="contact.contact_ref_no || contact.contact_detail"
                class="flex items-center justify-between p-4 bg-gray-50 rounded-lg border border-gray-200 hover:bg-gray-100 transition-colors">
                <div class="flex items-center gap-3">
                  <div class="bg-red-100 p-2 rounded-lg">
                    <svg class="h-5 w-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div>
                    ...
                    <p class="text-sm text-gray-600">{{ contact.contact_type }} {{ contact.contact_sub_type ? '- ' +
                      contact.contact_sub_type : '' }}</p>
                  </div>
                </div>
      ...
      <div v-if="searched && !clientData && !pending && !error" class="text-center py-12">
        <svg class="mx-auto h-16 w-16 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0120 12c0-4.411-3.589-8-8-8s-8 3.589-8 8c0 2.152.851 4.165 2.343 5.657l1.414-1.414A6 6 0 0112 6a6 6 0 016 6 6 6 0 01-1.757 4.243l-1.414 1.414z">
          </path>
        </svg>
...
const isAuthenticated = computed(() => {
  return !!localStorage.getItem('lbb_auth_token');
});
...
const searchClientInfo = async () => {
  ...
  if (!isAuthenticated.value) {
    ...
    error.value = 'Please login to search client information';
    return;
  }
  ...
  try {
    ...
    if (err.status === 401) {
      localStorage.removeItem('lbb_auth_token');
      localStorage.removeItem('lbb_user');
      showLoginModal.value = true;
...
const addAccountToMobile = async (account) => {
  ...
  if (!isAuthenticated.value) {
    showLoginModal.value = true;
    error.value = 'Please login to add account';
    return;
  }
...
const handleLogin = async () => {
  ...
  try {
    ...
    if (response.token) {
      ...
      localStorage.setItem('lbb_auth_token', response.token);
      localStorage.setItem('lbb_user', JSON.stringify(response.user));
